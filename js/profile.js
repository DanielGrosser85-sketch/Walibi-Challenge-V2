...
window.WALIBI_AVATAR_PRESETS = [
  ...
  { id: "bender", name: "Bender", icon: "🤖", url: "assets/avatar_bender.jpg" },
  { id: "trichter_thor", name: "Trichter-Thor", icon: "⚡", url: "assets/avatar_trichter_thor.jpg" },
  { id: "absturz_mario", name: "Absturz-Mario", icon: "👨🏻‍🔧", url: "assets/avatar_absturz_mario.jpg" },
  { id: "achterbahn_papst", name: "Achterbahn-Papst", icon: "⛪", url: "assets/avatar_achterbahn_papst.jpg" },
  { id: "schnapsdrossel_susi", name: "Schnapsdrossel Susi", icon: "🦉", url: "assets/avatar_schnapsdrossel_susi.jpg" },
  { id: "scooby_doo", name: "Scooby-Doo", icon: "🐕", url: "assets/avatar_scooby_doo.jpg" },
  { id: "meister_proper", name: "Meister Proper", icon: "🧽", url: "assets/avatar_meister_proper.jpg" },
  { id: "kaeptn_pegel", name: "Käpt'n Pegel", icon: "🏴‍☠️", url: "assets/avatar_kaeptn_pegel.jpg" },
  { id: "schluckspecht_schorsch", name: "Schluckspecht Schorsch", icon: "🦅", url: "assets/avatar_schluckspecht_schorsch.jpg" },
  { id: "promille_pikachu", name: "Promille-Pikachu", icon: "⚡", url: "assets/avatar_promille_pikachu.jpg" },
  { id: "kaeptn_blaubaer", name: "Käpt'n Blaubär", icon: "🐻", url: "assets/avatar_kaeptn_blaubaer.jpg" },
  { id: "li_la_launebaer", name: "Li-La-Launebär", icon: "🐻", url: "assets/avatar_li_la_launebaer.jpg" },
...
const ProfileModule = {
  ...
  tempCreateStats: { magen: 5, blase: 5, geduld: 5, durst: 5, adrenalin: 5 },
  tempEditStats: { magen: 5, blase: 5, geduld: 5, durst: 5, adrenalin: 5 },
  ...
  openMyProfileModal() {
    ...
    this.setEditGender(user.gender || "male");
    ...
    // RPG Klassen-Auswahl (20 Klassen) initialisieren
    const classSelect = document.getElementById("editProfileClassSelect");
    if (classSelect && window.RPG_CLASSES) {
      classSelect.innerHTML = window.RPG_CLASSES.map(c => `
        <option value="${c.id}" ${(user.rpgClass || 'brecher') === c.id ? 'selected' : ''}>
          ${c.icon} ${c.name} (${c.tagline})
        </option>
      `).join("");
      this.handleClassSelectChange(user.rpgClass || "brecher");
    }

    // 5 Fun-Statuswerte initialisieren
    this.tempEditStats = { ...(user.stats || { magen: 5, blase: 5, geduld: 5, durst: 5, adrenalin: 5 }) };
    this.renderRpgStats();

    // Freigeschaltete Titel rendern
    this.renderUnlockedTitles(user);

    // Pegel-Turnier Opt-in Checkbox
    const optInEl = document.getElementById("editProfilePromilleOptIn");
    if (optInEl) {
      optInEl.checked = user.promilleOptIn !== false;
    }
  ...
  setEditGender(gender) {
    if (window.GameAudio) window.GameAudio.playClick();
    const input = document.getElementById("editProfileGender");
    if (input) input.value = gender;
    ["Male", "Female"].forEach(k => {
      const btn = document.getElementById(`editGender${k}`);
      if (btn) {
        if (k.toLowerCase() === gender.toLowerCase()) {
  ...
  setNewGender(gender) {
    if (window.GameAudio) window.GameAudio.playClick();
    const input = document.getElementById("newPlayerGender");
    if (input) input.value = gender;
    ["Male", "Female"].forEach(k => {
      const btn = document.getElementById(`newGender${k}`);
      if (btn) {
        if (k.toLowerCase() === gender.toLowerCase()) {
          btn.classList.add("active");
          btn.style.boxShadow = "0 0 10px currentColor";
  ...
  handleNewPlayerClassChange(classId) {
    const classObj = (window.RPG_CLASSES || []).find(c => c.id === classId) || (window.RPG_CLASSES && window.RPG_CLASSES[0]);
    if (!classObj) return;

    const infoBox = document.getElementById("newPlayerClassInfoBox");
    const tagEl = document.getElementById("newPlayerClassCategoryTag");

    if (tagEl) {
      tagEl.textContent = (classObj.category || 'ACTION').toUpperCase();
    }
    if (infoBox) {
      infoBox.innerHTML = `
        <div style="display: flex; align-items: center; justify-content: space-between; margin-bottom: 4px;">
          <strong style="color: var(--walibi-yellow); font-size: 13px;">${classObj.icon} ${classObj.name}</strong>
          <span style="font-size: 11px; color: #38bdf8; font-weight: 800;">„${classObj.tagline}“</span>
        </div>
        <p style="color: #cbd5e1; font-size: 11.5px; margin-bottom: 6px; line-height: 1.35;">${classObj.desc}</p>
        <div style="background: rgba(255,204,0,0.15); border: 1px solid var(--walibi-yellow); color: var(--walibi-yellow); border-radius: 6px; padding: 4px 8px; font-size: 10.5px; font-weight: 800; display: inline-block;">
          ⚡ Buff: ${classObj.buff}
        </div>
      `;
    }
  ...
  renderNewPlayerRpgStats() {
    const container = document.getElementById("newPlayerRpgStatsContainer");
    if (!container) return;

    const statsDefs = window.RPG_STATS || [];
    container.innerHTML = statsDefs.map(s => {
      const val = Math.max(1, Math.min(10, Number(this.tempCreateStats && this.tempCreateStats[s.id] !== undefined ? this.tempCreateStats[s.id] : 5)));
      const percent = (val / 10) * 100;
      return `
        <div class="rpg-stat-row" style="background: rgba(0,0,0,0.4); border: 1px solid rgba(255,255,255,0.1); border-radius: 10px; padding: 8px 10px;">
          <div style="display: flex; justify-content: space-between; align-items: center; margin-bottom: 4px;">
            <div style="display: flex; align-items: center; gap: 6px;">
              <span style="font-size: 16px;">${s.icon}</span>
              <div>
                <strong style="font-size: 12px; color: #fff;">${s.name}</strong>
                <div style="font-size: 10px; color: var(--text-dim); line-height: 1.2;">${s.desc}</div>
              </div>
            </div>
            <div style="display: flex; align-items: center; gap: 6px;">
              <button type="button" class="stat-stepper-btn" onclick="ProfileModule.adjustNewPlayerStat('${s.id}', -1)" style="width: 28px; height: 28px; background: #334155; border: 1px solid #475569; color: #fff; border-radius: 6px; font-weight: 900; cursor: pointer;">-</button>
              <span style="font-size: 13px; font-weight: 900; color: ${s.color}; min-width: 32px; text-align: center;">${val}/10</span>
              <button type="button" class="stat-stepper-btn" onclick="ProfileModule.adjustNewPlayerStat('${s.id}', 1)" style="width: 28px; height: 28px; background: rgba(255,204,0,0.25); border: 1px solid var(--walibi-yellow); color: var(--walibi-yellow); border-radius: 6px; font-weight: 900; cursor: pointer;">+</button>
            </div>
          </div>
          <div class="rpg-stat-bar-bg" style="height: 6px; background: rgba(255,255,255,0.1); border-radius: 999px; overflow: hidden; margin-top: 4px;">
            <div class="rpg-stat-bar-fill" style="width: ${percent}%; height: 100%; background: ${s.color}; border-radius: 999px; transition: width 0.2s ease;"></div>
          </div>
        </div>
      `;
    }).join("");
  ...
  adjustNewPlayerStat(statId, delta) {
    if (window.GameAudio) window.GameAudio.playClick();
    if (!this.tempCreateStats) this.tempCreateStats = { magen: 5, blase: 5, geduld: 5, durst: 5, adrenalin: 5 };
    const cur = Number(this.tempCreateStats[statId] !== undefined ? this.tempCreateStats[statId] : 5);
    this.tempCreateStats[statId] = Math.max(1, Math.min(10, cur + delta));
    this.renderNewPlayerRpgStats();
  },

  handleClassSelectChange(classId) {
    const classObj = (window.RPG_CLASSES || []).find(c => c.id === classId) || (window.RPG_CLASSES && window.RPG_CLASSES[0]);
    if (!classObj) return;

    const infoBox = document.getElementById("myProfileClassInfoBox");
    const tagEl = document.getElementById("myProfileClassCategoryTag");
  ...
  renderRpgStats() {
    const container = document.getElementById("myProfileRpgStatsContainer");
    if (!container) return;

    const statsDefs = window.RPG_STATS || [];
    container.innerHTML = statsDefs.map(s => {
      const val = Math.max(1, Math.min(10, Number(this.tempEditStats && this.tempEditStats[s.id] !== undefined ? this.tempEditStats[s.id] : 5)));
      const percent = (val / 10) * 100;
      return `
        <div class="rpg-stat-row" style="background: rgba(0,0,0,0.4); border: 1px solid rgba(255,255,255,0.1); border-radius: 10px; padding: 8px 10px;">
          <div style="display: flex; justify-content: space-between; align-items: center; margin-bottom: 4px;">
            ...
            <div style="display: flex; align-items: center; gap: 6px;">
              <button type="button" class="stat-stepper-btn" onclick="ProfileModule.adjustStat('${s.id}', -1)" style="width: 28px; height: 28px; background: #334155; border: 1px solid #475569; color: #fff; border-radius: 6px; font-weight: 900; cursor: pointer;">-</button>
              <span style="font-size: 13px; font-weight: 900; color: ${s.color}; min-width: 32px; text-align: center;">${val}/10</span>
              <button type="button" class="stat-stepper-btn" onclick="ProfileModule.adjustStat('${s.id}', 1)" style="width: 28px; height: 28px; background: rgba(255,204,0,0.25); border: 1px solid var(--walibi-yellow); color: var(--walibi-yellow); border-radius: 6px; font-weight: 900; cursor: pointer;">+</button>
            </div>
          </div>
          <div class="rpg-stat-bar-bg" style="height: 6px; background: rgba(255,255,255,0.1); border-radius: 999px; overflow: hidden; margin-top: 4px;">
            <div class="rpg-stat-bar-fill" style="width: ${percent}%; height: 100%; background: ${s.color}; border-radius: 999px; transition: width 0.2s ease;"></div>
  ...
  adjustStat(statId, delta) {
    if (window.GameAudio) window.GameAudio.playClick();
    if (!this.tempEditStats) this.tempEditStats = { magen: 5, blase: 5, geduld: 5, durst: 5, adrenalin: 5 };
    const cur = Number(this.tempEditStats[statId] !== undefined ? this.tempEditStats[statId] : 5);
    this.tempEditStats[statId] = Math.max(1, Math.min(10, cur + delta));
    this.renderRpgStats();
  },

  renderUnlockedTitles(user) {
    const container = document.getElementById("myProfileUnlockedTitlesContainer");
    const countEl = document.getElementById("myProfileTitlesCount");
    if (!container) return;

    const userTitleIds = (user && Array.isArray(user.unlockedTitles)) ? user.unlockedTitles : [];
    ...
    if (unlocked.length === 0) {
      container.innerHTML = `
        <div style="display: flex; align-items: center; gap: 10px; background: rgba(0,0,0,0.35); border: 1.5px dashed rgba(255,255,255,0.15); border-radius: 10px; padding: 10px;">
          <span style="font-size: 22px;">🔒✨</span>
          <div style="font-size: 11px; color: var(--text-muted); line-height: 1.35;">
            <strong style="color: #94a3b8;">Noch keine Titel freigeschaltet!</strong><br/>
            Schalte Titel automatisch durch Coaster-Fahrten, Promille-Messungen (z. B. ab 0,3‰) oder Challenges frei.
          </div>
        </div>
      `;
      return;
    ...
    container.innerHTML = `
      <div style="display: flex; flex-direction: column; gap: 6px;">
        ${unlocked.map(t => `
          <div class="unlocked-title-chip" style="display: flex; align-items: center; justify-content: space-between; background: linear-gradient(135deg, rgba(255,204,0,0.15), rgba(0,0,0,0.5)); border: 1.5px solid var(--walibi-yellow); border-radius: 8px; padding: 6px 10px;">
            <div style="display: flex; align-items: center; gap: 8px;">
              <span style="font-size: 18px;">${t.icon || '🎖️'}</span>
              <div>
                <strong style="font-size: 12px; color: var(--walibi-yellow);">${t.title}</strong>
                <div style="font-size: 10.5px; color: var(--text-muted); line-height: 1.2;">${t.desc || ''}</div>
              </div>
            </div>
            <span style="background: rgba(16,185,129,0.25); border: 1px solid #10b981; color: #34d399; font-size: 9px; font-weight: 900; padding: 2px 6px; border-radius: 4px; text-transform: uppercase;">Aktiv</span>
          </div>
        `).join("")}
      </div>
    `;
  ...
  openCreatePlayerModal(preselectedAvatar = null) {
    ...
    if (createModal) {
      ...
      // Geschlecht auf Männlein vorbelegen
      this.setNewGender("male");

      // 20 RPG Klassen initialisieren
      const classSelect = document.getElementById("newPlayerClass");
      if (classSelect && window.RPG_CLASSES) {
        classSelect.innerHTML = window.RPG_CLASSES.map(c => `<option value="${c.id}">${c.icon} ${c.name} (${c.tagline})</option>`).join("");
        this.handleNewPlayerClassChange("brecher");
      }

      // 5 Fun-Statuswerte initialisieren
      this.tempCreateStats = { magen: 5, blase: 5, geduld: 5, durst: 5, adrenalin: 5 };
      this.renderNewPlayerRpgStats();

      // Pegel-Turnier Opt-In Checkbox
      const optInEl = document.getElementById("newPlayerPromilleOptIn");
  ...
  renderPlayersList() {
    ...
    listContainer.innerHTML = allPlayers.map(p => {
      ...
      return `
        <div class="profile-card ${isSelected ? 'active' : ''}" onclick="ProfileModule.selectExistingPlayer('${p.id}')">
          <img src="${p.avatar || this.generateDefaultAvatar(p.name)}" class="header-avatar" style="width: 44px; height: 44px; border: 2px solid ${isSelected ? 'var(--walibi-yellow)' : 'rgba(255,255,255,0.2)'};" />
          <div style="flex: 1; min-width: 0;">
            <div style="display: flex; align-items: center; gap: 6px;">
              <strong style="font-size: 14px; color: #fff; white-space: nowrap; overflow: hidden; text-overflow: ellipsis;">${p.name}</strong>
              ${isGrossek ? '<span class="admin-crown-badge">👑 Admin</span>' : ''}
            </div>
            <div style="font-size: 11px; color: var(--text-muted); display: flex; align-items: center; gap: 6px; margin-top: 2px;">
              <span class="feed-house-tag" style="padding: 1px 6px; font-size: 9.5px;">${p.house || 'Haus 1'}</span>
              <span style="color: var(--walibi-yellow); font-size: 10.5px;">${userClass.icon} ${userClass.name}</span>
            </div>
          </div>
          <div style="text-align: right;">
            <div style="font-size: 14px; font-weight: 900; color: var(--walibi-yellow);">${p.points || 0} Pkt</div>
            <div style="font-size: 10px; color: var(--text-muted);">${(p.completedQuests || []).length} Quests</div>
          </div>
        </div>
      `;
    }).join("");
  ...
  selectExistingPlayer(playerId) {
    const player = window.store.state.players.find(p => p.id === playerId);
    if (!player) return;

    if (window.GameAudio) window.GameAudio.playReward();

    const isGrossek = player.name.toLowerCase() === "grossek";
    if (!isGrossek) {
      this.isAdmin = false;
      localStorage.setItem("walibi_access_code", "6969");
    } else {
  ...
  setupProfileModals() {
    ...
    if (editForm) {
      editForm.onsubmit = async (e) => {
        ...
        const nameInp = document.getElementById("editProfileNameInput");
        const houseSelect = document.getElementById("editProfileHouseSelect");
        const genderInp = document.getElementById("editProfileGender");
        const classSelect = document.getElementById("editProfileClassSelect");
        const optInEl = document.getElementById("editProfilePromilleOptIn");
        ...
        const newGender = genderInp ? genderInp.value : (user ? user.gender : "male");
        const newAvatar = this.capturedEditAvatarBase64 || (user ? user.avatar : this.generateDefaultAvatar(newName));
        const newRpgClass = classSelect ? classSelect.value : (user ? user.rpgClass : "brecher");
        const newPromilleOptIn = optInEl ? optInEl.checked : true;
        const newStats = this.tempEditStats || (user ? user.stats : { magen: 5, blase: 5, geduld: 5, durst: 5, adrenalin: 5 });
        ...
        if (!user) {
          ...
          const newPlayer = await window.store.addPlayer(newName, newHouse, newAvatar, newGender, newRpgClass, newStats, newPromilleOptIn);
          ...
          await window.store.updateProfile(user.id, {
            ...
            gender: newGender,
            avatar: newAvatar,
            rpgClass: newRpgClass,
            stats: newStats,
            promilleOptIn: newPromilleOptIn
          });
        }
    ...
    if (createForm) {
      createForm.onsubmit = async (e) => {
        ...
        const gender = genderInp ? genderInp.value : "male";
        const chosenClass = classSelect ? classSelect.value : "brecher";
        const promilleOptIn = optInEl ? optInEl.checked : true;
        const stats = this.tempCreateStats || { magen: 5, blase: 5, geduld: 5, durst: 5, adrenalin: 5 };

        if (!name) return;

        const avatar = this.capturedNewAvatarBase64 || this.generateDefaultAvatar(name);
        const newPlayer = await window.store.addPlayer(name, house, avatar, gender, chosenClass, stats, promilleOptIn);
