...
class AppStore {
  ...
  syncCompletedQuestsToRideCounts(player) {
    if (!player) return;
    if (!player.rideCounts || typeof player.rideCounts !== 'object') player.rideCounts = {};
    const questToAttrMap = {
      // Coasters
      "coaster_yoy": ["attr_yoy_thrill", "attr_yoy_chill"],
      "coaster_yoy_chill": ["attr_yoy_chill"],
      "coaster_yoy_thrill": ["attr_yoy_thrill"],
      ...
      "coaster_condor_nogrip": ["attr_condor"],
      "coaster_xpress": ["attr_xpress"],
      "coaster_eat_my_dust": ["attr_eat_my_dust"],
      "coaster_drako": ["attr_drako"],
      "coaster_speedrun": ["attr_untamed", "attr_goliath", "attr_lost_gravity"],
      "coaster_promille_titan": ["attr_untamed", "attr_goliath"],

      // Water Rides
      "water_crazy_river": ["attr_crazy_river"],
      "water_el_rio": ["attr_el_rio_grande"],
      "water_el_rio_grande": ["attr_el_rio_grande"],
      "water_splash_battle": ["attr_splash_battle"],

      // Flat Rides & Thrill Rides
      "ride_gforce_after_chug": ["attr_g_force"],
      "ride_g_force": ["attr_g_force"],
      "ride_gforce": ["attr_g_force"],
      "ride_tomahawk_roulette": ["attr_tomahawk"],
      "ride_tomahawk_circle_walk": ["attr_tomahawk"],
      "ride_tomahawk": ["attr_tomahawk"],
      "ride_spaceshot_silence": ["attr_space_shot"],
      "ride_space_shot": ["attr_space_shot"],
      "ride_space_kidz": ["attr_space_kidz"],
      "ride_blast_line_walk": ["attr_blast"],
      "ride_blast": ["attr_blast"],
      "ride_excalibur": ["attr_excalibur"],
      "ride_spinning_vibe": ["attr_spinning_vibe"],
      "ride_super_swing": ["attr_super_swing"],
      "ride_grande_roue": ["attr_grande_roue"],
      "ride_merlins_castle": ["attr_merlins_castle"],
      "ride_skydiver": ["attr_skydiver"],
      "ride_pavillon_de_the": ["attr_pavillon_de_the"],
      "ride_wind_dancer": ["attr_wind_dancer"],
      "ride_mini_taxis": ["attr_mini_taxis"],
      "ride_los_sombreros": ["attr_los_sombreros"],
      "ride_tour_des_jardins": ["attr_tour_des_jardins"],
      "ride_merrie_goround": ["attr_merrie_goround"],
      "ride_walibi_express": ["attr_walibi_express"]
    };

    if (Array.isArray(player.completedQuests)) {
      player.completedQuests.forEach(qId => {
        const targetAttrs = questToAttrMap[qId];
        if (targetAttrs) {
          targetAttrs.forEach(aId => {
            if (!player.rideCounts[aId] || Number(player.rideCounts[aId]) < 1) {
              player.rideCounts[aId] = 1;
            }
          });
        }
      });
      ...
      players: [
        {
          ...
          gender: "male",
          rpgClass: "brecher",
          stats: { magen: 5, blase: 5, geduld: 5, durst: 5, adrenalin: 5 },
          unlockedTitles: [],
          promilleLogs: [],
          promilleOptIn: true
        }
      ...
      if (data) {
        ...
        if (parsed && typeof parsed === "object") {
          ...
          if (Array.isArray(parsed.players)) {
            parsed.players.forEach(p => this.syncCompletedQuestsToRideCounts(p));
          }
  ...
  getStateFingerprint(db) {
    if (!db) return "";
    try {
      const playersPart = (db.players || []).map(p => `${p.id}:${p.points}:${p.sympathyPoints || 0}:${p.drinksCount}:${(p.completedQuests||[]).length}:${p.gutGlaubenCount||0}:${p.name}:${p.avatar}:${p.gender||'male'}:${p.rpgClass||'brecher'}:${(p.promilleLogs||[]).length}:${(p.unlockedTitles||[]).length}`).join("|");
  ...
  mergeServerState(serverDb) {
    ...
    serverPlayers.forEach(sp => {
      ...
      if (playerMap.has(sp.id)) {
        ...
        const merged = {
          ...
          gender: sp.gender || localP.gender || "male",
          rpgClass: sp.rpgClass || localP.rpgClass || "brecher",
          stats: sp.stats || localP.stats || { magen: 5, blase: 5, geduld: 5, durst: 5, adrenalin: 5 },
          unlockedTitles: Array.from(new Set([...(localP.unlockedTitles || []), ...(sp.unlockedTitles || [])])),
          promilleOptIn: sp.promilleOptIn !== undefined ? sp.promilleOptIn : (localP.promilleOptIn !== undefined ? localP.promilleOptIn : true)
        };

        // Promille-Logs mergen
        const pmlMap = new Map();
        (localP.promilleLogs || []).forEach(l => { if (l && l.id) pmlMap.set(l.id, l); });
        (sp.promilleLogs || []).forEach(l => { if (l && l.id) pmlMap.set(l.id, l); });
        merged.promilleLogs = Array.from(pmlMap.values()).sort((a, b) => new Date(a.timestamp || 0) - new Date(b.timestamp || 0));
    ...
    mergedPlayers.forEach(p => this.syncCompletedQuestsToRideCounts(p));
  ...
  async addPlayer(name, house = "Haus 1", avatar = null, gender = "male", rpgClass = "brecher", stats = null, promilleOptIn = true) {
    ...
    const newPlayer = {
      ...
      avatar: avatar,
      gender: gender || "male",
      rpgClass: rpgClass || "brecher",
      stats: stats || { magen: 5, blase: 5, geduld: 5, durst: 5, adrenalin: 5 },
      unlockedTitles: [],
      promilleLogs: [],
      promilleOptIn: promilleOptIn !== false,
      points: 0,
      drinksCount: 0,
      completedQuests: [],
      completedSideQuests: [],
  ...
  async updateProfile(userId, updates) {
    ...
    if (idx >= 0) {
      ...
      this.state.players.push({
        ...
        rpgClass: updates.rpgClass || "brecher",
        stats: updates.stats || { magen: 5, blase: 5, geduld: 5, durst: 5, adrenalin: 5 },
        unlockedTitles: [],
        promilleLogs: [],
        promilleOptIn: true,
        points: 0,
  ...
  // --- RPG-CHARAKTERBOGEN AKTUALISIEREN (KLASSE & 5 STATS) ---
  async updateRpgProfile(userId, rpgClass, stats) {
    const player = this.state.players.find(p => p.id === userId);
    if (!player) return;

    if (rpgClass) player.rpgClass = rpgClass;
    if (stats && typeof stats === 'object') {
      player.stats = { ...(player.stats || { magen: 5, blase: 5, geduld: 5, durst: 5, adrenalin: 5 }), ...stats };
    }
    ...
    this.saveLocalState();
    if (window.ProfileModule && window.ProfileModule.updateHeaderProfile) window.ProfileModule.updateHeaderProfile();
  ...
  // --- TITEL & ERRUNGENSCHAFTEN FREISCHALTEN ---
  unlockTitle(userId, titleObj) {
    const player = this.state.players.find(p => p.id === userId);
    if (!player) return false;

    if (!Array.isArray(player.unlockedTitles)) player.unlockedTitles = [];
    if (player.unlockedTitles.includes(titleObj.id)) return false;

    player.unlockedTitles.push(titleObj.id);
    ...
    if (this.state.currentUser && this.state.currentUser.id === userId) {
      ...
      if (window.app && window.app.showToast) {
        window.app.showToast(`🏆 <strong>Neuer Titel freigeschaltet!</strong><br/>${titleObj.icon || '🎖️'} <strong>${titleObj.title}</strong>: <em>${titleObj.desc || ''}</em>`);
      }
    }

    this.saveLocalState();
    this.updateProfile(userId, { unlockedTitles: player.unlockedTitles });
    return true;
  }

  // Automatische Überprüfung von Aktions-Titeln
  checkAndUnlockActionTitles(userId) {
    const player = this.state.players.find(p => p.id === userId);
    if (!player) return;
    ...
    // 1. Untamed-Bezwinger
    if ((Number(rideCounts["attr_untamed"] || 0) >= 1 || completedQuests.includes("coaster_untamed"))) {
      this.unlockTitle(userId, { id: "title_untamed_bezwinger", title: "Untamed-Bezwinger", icon: "🪵", desc: "Hybrid-Coaster Untamed bezwungen." });
    }

    // 2. First-Drop-Survivor
    if (Number(rideCounts["attr_goliath"] || 0) >= 1 || Number(rideCounts["attr_lost_gravity"] || 0) >= 1 || Number(rideCounts["attr_yoy_thrill"] || 0) >= 1 || completedQuests.includes("coaster_goliath")) {
      this.unlockTitle(userId, { id: "title_first_drop_survivor", title: "First-Drop-Survivor", icon: "🎢", desc: "Den Mega-Drop von Goliath oder Lost Gravity gemeistert." });
    }

    // 3. Wasser-Magnet
    if (Number(rideCounts["attr_crazy_river"] || 0) >= 1 || Number(rideCounts["attr_el_rio_grande"] || 0) >= 1 || completedQuests.includes("water_crazy_river") || completedQuests.includes("water_el_rio")) {
      this.unlockTitle(userId, { id: "title_wasser_magnet", title: "Wasser-Magnet", icon: "🌊", desc: "Crazy River oder El Rio Grande gefahren und nass geworden." });
    }

    // 4. Pommes-Diplomat
    if (completedQuests.includes("visitor_swap_fries") || completedQuests.includes("fun_fritte_spezial") || completedQuests.includes("visitor_snack_smelltest")) {
      this.unlockTitle(userId, { id: "title_pommes_diplomat", title: "Pommes-Diplomat", icon: "🍟", desc: "Den Pommes-Tauschhandel oder Frikandel-Test bestanden." });
    }

    // 5. Schaumkronen-König
    if (Number(drinksDetail.beer || 0) >= 5) {
      this.unlockTitle(userId, { id: "title_schaumkronen_koenig", title: "Schaumkronen-König", icon: "🍺", desc: "Mindestens 5 Biere auf der Tour registriert." });
    }

    // 6. Schnappschuss-Legende
    const myPostsCount = (this.state.feed || []).filter(f => f.userId === userId).length;
    if (myPostsCount >= 5) {
      this.unlockTitle(userId, { id: "title_schnappschuss_legende", title: "Schnappschuss-Legende", icon: "📸", desc: "Mindestens 5 Fotos oder Videos im Live-Feed geteilt." });
    }

    // 7. Ehren-Zeuge
    let myConfirmedWitnessesCount = 0;
    (this.state.feed || []).forEach(f => {
      if (f.witnesses && f.witnesses.some(w => w.userId === userId && w.confirmed)) {
        myConfirmedWitnessesCount++;
      }
    });
    if (myConfirmedWitnessesCount >= 3) {
      this.unlockTitle(userId, { id: "title_ehren_zeuge", title: "Ehren-Zeuge", icon: "👁️", desc: "Mindestens 3 Quests von Mitspielern als Zeuge bestätigt." });
    }

    // 8. Thronanwärter (Top 3 im Gesamtranking)
    const sorted = this.getSortedPlayers();
    const rank = sorted.findIndex(p => p.id === userId) + 1;
    if (rank > 0 && rank <= 3 && (player.points || 0) > 0) {
      this.unlockTitle(userId, { id: "title_thronanwaerter", title: "Mr./Mrs. Walibi Thronanwärter", icon: "👑", desc: "Einen Platz unter den Top 3 im Gesamtranking erreicht." });
    }
  }

  // --- PROMILLE-MESSUNGEN LOGGEN & VERWALTEN ---
  async logPromilleReading(userId, value, timeStr = null, note = "", photoBase64 = null, shareToFeed = false) {
    const player = this.state.players.find(p => p.id === userId);
    if (!player) return null;

    if (!Array.isArray(player.promilleLogs)) player.promilleLogs = [];
    if (!Array.isArray(player.unlockedTitles)) player.unlockedTitles = [];

    const parsedVal = parseFloat(value);
    const numVal = isNaN(parsedVal) ? 0.0 : Math.round(parsedVal * 100) / 100;
    const nowTimeStr = timeStr || new Date().toLocaleTimeString('de-DE', { hour: '2-digit', minute: '2-digit' });

    const logEntry = {
      id: "pml_" + Date.now() + "_" + Math.random().toString(36).substr(2, 4),
      value: numVal,
    ...
    player.promilleLogs.push(logEntry);
    player.promilleLogs.sort((a, b) => new Date(a.timestamp || 0) - new Date(b.timestamp || 0));

    // +10 XP Belohnung fürs Pusten & Messen (2x bei Happy Hour)
    const earnedPoints = 10 * this.getPointsMultiplier();
    player.points = (player.points || 0) + earnedPoints;

    if (this.state.currentUser && this.state.currentUser.id === userId) {
      this.state.currentUser.points = player.points;
      this.state.currentUser.promilleLogs = [...player.promilleLogs];
    }

    // Automatische Promille-Meilenstein-Prüfung
    const milestones = window.PROMILLE_MILESTONES || [
      { threshold: 0.3, id: "title_promille_03", title: "Warm-Trinker", icon: "🍻", desc: "0,3‰ erreicht" },
      { threshold: 0.5, id: "title_promille_05", title: "Hobby-Alkoholiker", icon: "🍺", desc: "0,5‰ erreicht" },
      { threshold: 0.8, id: "title_promille_08", title: "Fahrgeschäft-Tauglichkeits-Prüfer", icon: "🎢", desc: "0,8‰ erreicht" },
      { threshold: 1.0, id: "title_promille_10", title: "Pegel-Kapitän", icon: "⚓", desc: "1,0‰ erreicht" },
      { threshold: 1.3, id: "title_promille_13", title: "Schluckspecht von Walibi", icon: "🦅", desc: "1,3‰ erreicht" },
      { threshold: 1.5, id: "title_promille_15", title: "Trunkenbold der Tafelrunde", icon: "⚔️", desc: "1,5‰ erreicht" },
      { threshold: 1.8, id: "title_promille_18", title: "Promille-Baron", icon: "👑", desc: "1,8‰ erreicht" },
      { threshold: 2.0, id: "title_promille_20", title: "Lebende Legende / Absoluter Härtetest", icon: "💀", desc: "2,0+‰ erreicht" }
    ];

    milestones.forEach(m => {
      if (numVal >= m.threshold && !player.unlockedTitles.includes(m.id)) {
        this.unlockTitle(userId, m);
      }
    });

    if (shareToFeed || photoBase64) {
      const feedItem = {
        id: "feed_promille_" + Date.now() + "_" + Math.random().toString(36).substr(2, 4),
        type: "promille",
        userId: player.id,
        userName: player.name,
        userAvatar: player.avatar,
        userHouse: player.house,
        text: `🚨 Offizielle Promille-Messung: ${numVal} ‰! ${logEntry.note ? `("${logEntry.note}")` : ''}`,
        photo: photoBase64 || null,
        points: earnedPoints,
        actualPointsAwarded: earnedPoints,
        isHappyHour: this.getPointsMultiplier() > 1,
        timestamp: new Date().toISOString(),
        reactions: { "🔥": [], "🍺": [], "👑": [], "💀": [], "👏": [] },
        comments: []
      };
    ...
    try {
      const res = await fetch("/api/promille/log", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({
          userId,
      ...
      if (res.ok) {
        const json = await res.json();
        if (json.player && json.player.promilleLogs) {
          player.promilleLogs = json.player.promilleLogs;
          if (this.state.currentUser && this.state.currentUser.id === userId) {
            this.state.currentUser.promilleLogs = [...player.promilleLogs];
          }
          this.saveLocalState();
      ...
      console.warn("Promille log offline fallback:", e);
    }

    return logEntry;
  }

  async deletePromilleReading(userId, logId) {
    const player = this.state.players.find(p => p.id === userId);
    if (!player || !Array.isArray(player.promilleLogs)) return;

    player.promilleLogs = player.promilleLogs.filter(l => l.id !== logId);
    ...
    try {
      await fetch("/api/promille/delete", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
  ...
  async setPromilleOptIn(userId, optIn) {
    const player = this.state.players.find(p => p.id === userId);
    if (!player) return;

    player.promilleOptIn = Boolean(optIn);
    if (this.state.currentUser && this.state.currentUser.id === userId) {
      this.state.currentUser.promilleOptIn = player.promilleOptIn;
    }

    this.saveLocalState();
    if (window.app && window.app.renderAllViews) window.app.renderAllViews();

    return this.updateProfile(userId, { promilleOptIn: player.promilleOptIn });
  }

  // --- ⏱️ HYPOTHETISCHER PROMILIE-WERT INKL. NATÜRLICHEM MENSCHLICHEM ABBAU (-0.12‰ / h) ---
  calculateDynamicPromille(player, targetTime = Date.now()) {
    if (!player) return "0.00";

    const targetMs = typeof targetTime === "number" ? targetTime : new Date(targetTime).getTime();
    const ELIMINATION_PER_HOUR = 0.12; // 0.12‰ pro Stunde (durchschnittliche menschliche Abbau-Rate)
    const ELIMINATION_PER_MS = ELIMINATION_PER_HOUR / 3600000;

    // 1. Wenn detaillierte Drinks-Historie vorhanden ist:
    if (Array.isArray(player.drinksHistory) && player.drinksHistory.length > 0) {
      const history = [...player.drinksHistory].sort((a, b) => new Date(a.timestamp).getTime() - new Date(b.timestamp).getTime());
      let currentPromille = 0.0;
      let lastTimeMs = null;

      for (const entry of history) {
        const entryTimeMs = new Date(entry.timestamp).getTime();
        if (entryTimeMs > targetMs) break;

        if (lastTimeMs !== null) {
          const elapsedMs = Math.max(0, entryTimeMs - lastTimeMs);
          currentPromille = Math.max(0.0, currentPromille - (elapsedMs * ELIMINATION_PER_MS));
        }

        let gain = 0.0;
        if (typeof entry.promille === "number") {
          gain = entry.promille;
        } else if (entry.type === "beer") {
          gain = 0.28;
        } else if (entry.type === "longdrink") {
          gain = 0.25;
        } else if (entry.type === "shot") {
          gain = 0.15;
        }

        currentPromille += gain;
        lastTimeMs = entryTimeMs;
      }

      if (lastTimeMs !== null) {
        const finalElapsedMs = Math.max(0, targetMs - lastTimeMs);
        currentPromille = Math.max(0.0, currentPromille - (finalElapsedMs * ELIMINATION_PER_MS));
      }

      return Math.max(0.0, currentPromille).toFixed(2);
    }

    // 2. Fallback: Keine drinksHistory, aber drinksDetail vorhanden
    const details = player.drinksDetail || { beer: 0, shot: 0, longdrink: 0 };
    const beer = details.beer || 0;
    const shot = details.shot || 0;
    const longdrink = details.longdrink || 0;
    const grossPromille = (beer * 0.28) + (shot * 0.15) + (longdrink * 0.25);

    if (grossPromille <= 0) return "0.00";

    if (player.lastDrinkTimestamp) {
      const lastMs = new Date(player.lastDrinkTimestamp).getTime();
      const elapsedMs = Math.max(0, targetMs - lastMs);
      const decayed = Math.max(0.0, grossPromille - (elapsedMs * ELIMINATION_PER_MS));
      return decayed.toFixed(2);
    }

    return grossPromille.toFixed(2);
  }

  // --- PEGEL-TURNIER LEADERBOARD (HIGHSCORE & KONSTANZ-KÖNIG) ---
  getPromilleLeaderboard(genderFilter = "all") {
    let activeTourneyPlayers = (this.state.players || []).filter(p => {
      return p.promilleOptIn !== false && Array.isArray(p.promilleLogs) && p.promilleLogs.length > 0;
    });

    if (genderFilter === "male") {
      activeTourneyPlayers = activeTourneyPlayers.filter(p => p.gender === "male" || !p.gender);
    } else if (genderFilter === "female") {
      activeTourneyPlayers = activeTourneyPlayers.filter(p => p.gender === "female");
    }

    // 1. Highscores: Höchster Einzel-Messwert des Tages
    const highscoreList = activeTourneyPlayers.map(p => {
      let maxLog = p.promilleLogs[0];
      p.promilleLogs.forEach(l => {
        if (Number(l.value || 0) > Number(maxLog.value || 0)) {
          maxLog = l;
        }
      });
      return {
        player: p,
        peakValue: Number(maxLog.value || 0),
        peakTime: maxLog.timeStr || "",
        peakNote: maxLog.note || "",
        logsCount: p.promilleLogs.length
      };
    }).sort((a, b) => b.peakValue - a.peakValue);

    // 2. Konstanz-Könige: Höchster mathematischer Durchschnittswert aller Messungen
    const consistencyList = activeTourneyPlayers.map(p => {
      const total = p.promilleLogs.reduce((sum, l) => sum + Number(l.value || 0), 0);
      const avg = p.promilleLogs.length > 0 ? (total / p.promilleLogs.length) : 0;
      const minVal = Math.min(...p.promilleLogs.map(l => Number(l.value || 0)));
      const maxVal = Math.max(...p.promilleLogs.map(l => Number(l.value || 0)));
    ...
    return {
      ...
      consistency: consistencyList,
      totalParticipants: activeTourneyPlayers.length
    };
  }
  ...
  async logRide(userId, attrId, delta = 1) {
    ...
    // Nach jeder Fahrt automatisch prüfen, ob Nebenquests & Titel freigeschaltet wurden
    this.checkAndAutoUnlockSideQuests(userId);
    this.checkAndUnlockActionTitles(userId);
  }
  ...
  async completeQuest(userId, questId, photoBase64, customTitle = null, customDesc = null, userComment = "", options = {}) {
    ...
    const {
      ...
      witnessIds = [],
      isFaithBased = false,
      measuredPromille = null,
      promilleBonus = 0,
      bossWinnerId = null,
      bossDuelCategory = null
    } = options;

    // End-Boss Finale: Punkte gehen an den Sieger
    let targetPlayer = player;
    if (quest.isEndBossQuest && bossWinnerId) {
      const winner = this.state.players.find(p => p.id === bossWinnerId);
      if (winner) targetPlayer = winner;
    }

    let basePoints = typeof outcomePoints === "number" ? outcomePoints : quest.points;
    if (promilleBonus > 0) {
      basePoints += promilleBonus;
    }
    let calculatedPoints = basePoints;

    if (isFaithBased && basePoints > 0) {
      calculatedPoints = Math.round(basePoints * 0.8); // 20% Ehren-Abzug
      targetPlayer.gutGlaubenCount = (targetPlayer.gutGlaubenCount || 0) + 1;
    }

    const multiplier = this.getPointsMultiplier();
    const actualPoints = calculatedPoints > 0 ? (calculatedPoints * multiplier) : calculatedPoints;
    const isEndBoss = quest.isEndBossQuest === true || quest.id === "boss_park_king";
    const requiresVoting = isEndBoss ? false : quest.requiresVoting === true;
    const hasWitnesses = Array.isArray(witnessIds) && witnessIds.length > 0;
    const requiresWitnessPending = isEndBoss ? false : (hasWitnesses && (quest.witnessRequirement === "required" || quest.requiresWitness === true));
    ...
    const witnesses = hasWitnesses ? witnessIds.map(wId => {
      ...
      return {
        ...
        confirmed: isEndBoss ? true : false,
        confirmedAt: isEndBoss ? new Date().toISOString() : null
      };
    }) : [];
    ...
    if (actualPoints < 0) {
      ...
      targetPlayer.points = Math.max(0, (targetPlayer.points || 0) + actualPoints);
    } else if (isEndBoss || (!requiresVoting && !requiresWitnessPending)) {
      initialPoints = actualPoints;
      targetPlayer.points = (targetPlayer.points || 0) + actualPoints;
    }

    if (!targetPlayer.completedQuests.includes(questId)) {
      targetPlayer.completedQuests.push(questId);
    }

    // Promille-Messwert im Spielerprofil protokollieren
    if (measuredPromille) {
      if (!Array.isArray(targetPlayer.promilleLogs)) targetPlayer.promilleLogs = [];
      targetPlayer.promilleLogs.push({
        promille: measuredPromille,
        timestamp: new Date().toISOString(),
        source: quest.title
      });
    }

    if (this.state.currentUser && this.state.currentUser.id === targetPlayer.id) {
      this.state.currentUser.points = targetPlayer.points;
      this.state.currentUser.completedQuests = [...targetPlayer.completedQuests];
      if (targetPlayer.gutGlaubenCount) this.state.currentUser.gutGlaubenCount = targetPlayer.gutGlaubenCount;
    }

    // AUTOMATISCHE FAHRTEN-ZÄHLUNG (COASTER & FLAT RIDES) FÜR DEN PARK-GUIDE
    if (!targetPlayer.rideCounts) targetPlayer.rideCounts = {};
    const questToAttrMap = {
      // Coasters
      "coaster_yoy": ["attr_yoy_thrill", "attr_yoy_chill"],
      "coaster_yoy_chill": ["attr_yoy_chill"],
    ...
    if (selectedOutcome && (selectedOutcome.attrId || selectedOutcome.id === "untamed" || selectedOutcome.id === "goliath")) {
      const chosenAttr = selectedOutcome.attrId || (selectedOutcome.id === "untamed" ? "attr_untamed" : "attr_goliath");
      targetPlayer.rideCounts[chosenAttr] = (targetPlayer.rideCounts[chosenAttr] || 0) + 1;
    } else {
      const targetAttrs = questToAttrMap[questId];
      if (targetAttrs) {
        targetAttrs.forEach(attrId => {
          targetPlayer.rideCounts[attrId] = (targetPlayer.rideCounts[attrId] || 0) + 1;
        });
      }
    }
    ...
    if (quest.isEndBossQuest && bossWinnerId) {
      const catName = bossDuelCategory === 'mr' ? 'Mr. Walibi' : (bossDuelCategory === 'mrs' ? 'Mrs. Walibi' : (bossDuelCategory === 'promille' ? 'Promille-König' : 'Gesamtsieg'));
      finalTitle = `👑 END-BOSS SIEG: ${targetPlayer.name} gewinnt das ${catName}-Finale!`;
      customDesc = `${targetPlayer.name} hat das epische End-Boss Duell gewonnen und sichert sich +70 Punkte!`;
    } else if (selectedOutcome && !customTitle) {
      finalTitle = `${quest.title} • ${selectedOutcome.label}`;
    }

    if (measuredPromille && !finalTitle.includes("‰")) {
      finalTitle += ` 📈 (${measuredPromille.toFixed(2)}‰ • +${promilleBonus} Pkt Bonus)`;
    }
    ...
    const payload = {
      ...
      bossWinnerId: bossWinnerId,
      bossDuelCategory: bossDuelCategory,
  ...
  async confirmWitness(feedItemId, witnessUserId) {
    ...
    this.checkAndUnlockActionTitles(witnessUserId);
  ...
  async logCounterItem(userId, itemId) {
    ...
    // Zeitstempel & Historie für physiologischen Promille-Abbau festhalten
    if (!Array.isArray(player.drinksHistory)) {
      player.drinksHistory = [];
    }
    let promilleGain = 0;
    if (itemId === "beer") promilleGain = 0.28;
    else if (itemId === "longdrink") promilleGain = 0.25;
    else if (itemId === "shot") promilleGain = 0.15;

    const nowIso = new Date().toISOString();
    player.drinksHistory.push({
      type: itemId,
      timestamp: nowIso,
      promille: promilleGain
    });
    player.lastDrinkTimestamp = nowIso;
    ...
    if (this.state.currentUser && (this.state.currentUser.id === userId || this.state.currentUser.id === player.id)) {
      ...
      this.state.currentUser.drinksHistory = [...player.drinksHistory];
      this.state.currentUser.lastDrinkTimestamp = player.lastDrinkTimestamp;
    }

    this.saveLocalState();
    this.checkAndUnlockActionTitles(player.id);
  ...
  // --- 🚨 MALUS-SKALA (STRAFPUNKTE BUCHEN) ---
  async logMalusItem(userId, malusId) {
    let player = this.state.players.find(p => p.id === userId);
    if (!player && this.state.currentUser && this.state.currentUser.id === userId) {
      player = this.state.currentUser;
      this.state.players.push(player);
    ...
    const malusItems = window.MALUS_ITEMS || [
      { id: "nein", name: "Ein Nein", icon: "🚫", points: -5, description: "Challenge / Getränk verweigert" },
      { id: "toilet", name: "Toilettengang", icon: "🚽", points: -10, description: "Schwache Blase / Pipi-Pause" },
      { id: "vomit", name: "Übergeben", icon: "🤮", points: -20, description: "Magen-Kapitulation / Kotz-Malus" }
    ];
    const item = malusItems.find(m => m.id === malusId) || { id: malusId, name: malusId, points: -5, icon: "⚠️" };

    const penaltyPoints = typeof item.points === "number" ? item.points : -5;
    player.points = Math.max(0, Number(player.points || 0) + penaltyPoints);

    if (!player.malusDetail) {
      player.malusDetail = { nein: 0, toilet: 0, vomit: 0 };
    }
    player.malusDetail[malusId] = Number(player.malusDetail[malusId] || 0) + 1;
    player.malusCount = Number(player.malusCount || 0) + 1;

    if (this.state.currentUser && (this.state.currentUser.id === userId || this.state.currentUser.id === player.id)) {
      this.state.currentUser.points = player.points;
      this.state.currentUser.malusCount = player.malusCount;
      this.state.currentUser.malusDetail = { ...player.malusDetail };
    }

    // Feed Item für die Malus-Strafe generieren
    const feedItem = {
      id: "feed_" + Date.now() + "_" + Math.random().toString(36).substr(2, 5),
      type: "malus",
      userId: player.id,
      ...
      userAvatar: player.avatar,
      userHouse: player.house,
      questTitle: `🚨 STRAFE: ${item.name} (${penaltyPoints} Pkt)`,
      questDescription: `${player.name} hat einen Malus kassiert: ${item.description || item.name} (${penaltyPoints} Punkte abgezogen)!`,
      questIcon: item.icon || "🚨",
      points: penaltyPoints,
      actualPointsAwarded: penaltyPoints,
      timestamp: new Date().toISOString(),
      reactions: { "🔥": [], "🍺": [], "👑": [], "💀": [], "👏": [] },
      comments: []
    };
  ...
  // --- HILFSMETHODEN ---
  getSortedPlayers(genderFilter = "all") {
    let players = [...this.state.players];
    if (genderFilter === "male") {
      players = players.filter(p => p.gender === "male" || !p.gender);
    } else if (genderFilter === "female") {
