...
// --- MAPPING VON QUEST-AUFGABEN AUF ATTRAKTIONEN (COASTER & FLAT RIDES) ---
const QUEST_TO_ATTR_MAP = {
  // Coasters
...
function syncPlayerQuestsToRides(player) {
  if (!player) return;
  ...
  if (Array.isArray(player.completedQuests)) {
    player.completedQuests.forEach(qId => {
      const targetAttrs = QUEST_TO_ATTR_MAP[qId];
      if (targetAttrs) {
  ...
  try {
    ...
    if (Array.isArray(db.players)) {
      db.players.forEach(p => {
        syncPlayerQuestsToRides(p);
        if (!p.gender) {
          p.gender = (p.name && p.name.toLowerCase() === "heidi") || (p.avatar && p.avatar.includes("prinzessin")) ? "female" : "male";
        }
      });
    }
    if (Array.isArray(db.feed)) {
      db.feed.forEach(f => {
        if (f.questId === "boss_park_king" && f.actualPointsAwarded === 0) {
          f.actualPointsAwarded = f.points || 70;
          f.witnessPending = false;
        }
      });
    }
  } catch (e) {
...
function handleApiRequest(pathname, req, res) {
    ...
    parseJsonBody(req, (err, data) => {
      ...
      if (!p) {
        p = {
          ...
          rpgClass: data.rpgClass || "brecher",
          stats: data.stats || { magen: 5, blase: 5, geduld: 5, durst: 5, adrenalin: 5 },
          unlockedTitles: Array.isArray(data.unlockedTitles) ? data.unlockedTitles : [],
          promilleLogs: Array.isArray(data.promilleLogs) ? data.promilleLogs : [],
          promilleOptIn: data.promilleOptIn !== undefined ? Boolean(data.promilleOptIn) : true
        };
        db.players.push(p);
      } else {
        ...
        if (data.rpgClass !== undefined) p.rpgClass = data.rpgClass;
        if (data.stats && typeof data.stats === 'object') p.stats = data.stats;
        if (Array.isArray(data.unlockedTitles)) p.unlockedTitles = data.unlockedTitles;
        if (Array.isArray(data.promilleLogs)) p.promilleLogs = data.promilleLogs;
        if (data.promilleOptIn !== undefined) p.promilleOptIn = Boolean(data.promilleOptIn);
      }
  ...
  // POST /api/promille/log (Messwert vom Promille-Testgerät eintragen + 10 XP)
  if (pathname === '/api/promille/log' && req.method === 'POST') {
    parseJsonBody(req, (err, data) => {
      if (err || !data.userId || data.value === undefined) {
        res.writeHead(400, { 'Content-Type': 'application/json' });
        res.end(JSON.stringify({ error: 'Ungültige Messwert-Daten' }));
        return;
      }
      ...
      if (data.photoBase64 && data.photoBase64.startsWith('data:image')) {
        photoUrl = saveMediaBase64(data.photoBase64, `promille_${player.id}`);
      }

      const val = parseFloat(data.value);
      const logEntry = {
        id: "pml_" + Date.now() + "_" + Math.random().toString(36).substr(2, 4),
        value: isNaN(val) ? 0.0 : Math.round(val * 100) / 100,
        timestamp: data.timestamp || new Date().toISOString(),
        timeStr: data.timeStr || new Date().toLocaleTimeString('de-DE', { hour: '2-digit', minute: '2-digit' }),
        note: (data.note || "").trim(),
        photo: photoUrl
      };
      ...
      // +10 XP Bonus pro Messung (2x bei Happy Hour)
      const earnedPoints = 10 * getPointsMultiplier();
      player.points = (player.points || 0) + earnedPoints;

      // Automatische Promille-Meilenstein-Prüfung
      const milestones = [
        { threshold: 0.3, id: "title_promille_03", title: "Warm-Trinker", icon: "🍻" },
        { threshold: 0.5, id: "title_promille_05", title: "Hobby-Alkoholiker", icon: "🍺" },
        { threshold: 0.8, id: "title_promille_08", title: "Fahrgeschäft-Tauglichkeits-Prüfer", icon: "🎢" },
        { threshold: 1.0, id: "title_promille_10", title: "Pegel-Kapitän", icon: "⚓" },
        { threshold: 1.3, id: "title_promille_13", title: "Schluckspecht von Walibi", icon: "🦅" },
        { threshold: 1.5, id: "title_promille_15", title: "Trunkenbold der Tafelrunde", icon: "⚔️" },
        { threshold: 1.8, id: "title_promille_18", title: "Promille-Baron", icon: "👑" },
        { threshold: 2.0, id: "title_promille_20", title: "Lebende Legende / Absoluter Härtetest", icon: "💀" }
      ];

      const newlyUnlockedTitles = [];
      milestones.forEach(m => {
        if (logEntry.value >= m.threshold && !player.unlockedTitles.includes(m.id)) {
          player.unlockedTitles.push(m.id);
          newlyUnlockedTitles.push(m);
        }
      });

      // Optional: Post in Live-Feed teilen wenn Notiz oder Foto vorhanden
      if (data.shareToFeed || photoUrl) {
        const feedItem = {
          ...
          text: `🚨 Offizielle Promille-Messung: ${logEntry.value} ‰! ${logEntry.note ? `("${logEntry.note}")` : ''}`,
          photo: photoUrl,
          points: earnedPoints,
          actualPointsAwarded: earnedPoints,
          isHappyHour: getPointsMultiplier() > 1,
      ...
      res.end(JSON.stringify({
        ...
        promilleLogs: player.promilleLogs
      }));
    });
    return;
  }

  // POST /api/promille/delete (Messwert löschen / korrigieren)
  if (pathname === '/api/promille/delete' && req.method === 'POST') {
    parseJsonBody(req, (err, data) => {
      if (err || !data.userId || !data.logId) {
        res.writeHead(400, { 'Content-Type': 'application/json' });
        res.end(JSON.stringify({ error: 'Ungültige Lösch-Daten' }));
        return;
      }
      ...
      if (Array.isArray(player.promilleLogs)) {
        player.promilleLogs = player.promilleLogs.filter(l => l.id !== data.logId);
      }
  ...
  if (pathname === '/api/quest/complete' && req.method === 'POST') {
    parseJsonBody(req, (err, data) => {
      ...
      let targetPlayer = db.players.find(p => p.id === (data.bossWinnerId || data.userId));
      if (!targetPlayer) {
        targetPlayer = db.players.find(p => p.id === data.userId);
      }
      if (!targetPlayer) {
        res.writeHead(404, { 'Content-Type': 'application/json' });
        res.end(JSON.stringify({ error: 'Spieler nicht gefunden' }));
      ...
      // Promille-Messwert protokollieren
      if (data.measuredPromille) {
        if (!Array.isArray(player.promilleLogs)) player.promilleLogs = [];
        player.promilleLogs.push({
          promille: data.measuredPromille,
          timestamp: new Date().toISOString(),
          source: data.questTitle || data.questId
        });
      }

      const isEndBoss = data.questId === "boss_park_king" || !!data.bossWinnerId;
      const requiresVoting = isEndBoss ? false : data.requiresVoting === true;
      const requiresWitnessPending = isEndBoss ? false : (data.requiresWitnessPending === true);
      const multiplier = getPointsMultiplier();
      ...
      // Automatische Fahrten-Zählung (Coaster & Flat Rides)
      if (!player.rideCounts) player.rideCounts = {};
      ...
      if (selectedOutcome && (selectedOutcome.attrId || selectedOutcome.id === "untamed" || selectedOutcome.id === "goliath")) {
        ...
        player.rideCounts[chosenAttr] = (player.rideCounts[chosenAttr] || 0) + 1;
      } else {
        const targetAttrs = QUEST_TO_ATTR_MAP[data.questId];
        if (targetAttrs) {
  ...
  if (pathname === '/api/counter/log' && req.method === 'POST') {
    parseJsonBody(req, (err, data) => {
      ...
      let promilleGain = 0;
      if (data.itemId === 'beer') promilleGain = 0.28;
      else if (data.itemId === 'longdrink') promilleGain = 0.25;
      else if (data.itemId === 'shot') promilleGain = 0.15;
      ...
      res.end(JSON.stringify({
        ...
        lastDrinkTimestamp: player.lastDrinkTimestamp
      }));
    });
    return;
  }

  // POST /api/malus/log (Malus-Skala / Strafpunkte)
  if (pathname === '/api/malus/log' && req.method === 'POST') {
    parseJsonBody(req, (err, data) => {
      if (err || !data.userId || !data.malusId) {
        res.writeHead(400, { 'Content-Type': 'application/json' });
        res.end(JSON.stringify({ error: 'Ungültige Malus-Daten' }));
        return;
      }
      ...
      player.points = Math.max(0, Number(player.points || 0) + points);

      if (!player.malusDetail) {
        player.malusDetail = { nein: 0, toilet: 0, vomit: 0 };
      }
      player.malusDetail[data.malusId] = Number(player.malusDetail[data.malusId] || 0) + 1;
      player.malusCount = Number(player.malusCount || 0) + 1;

      if (data.feedItem) {
        db.feed.unshift(data.feedItem);
      }
      ...
      res.end(JSON.stringify({
        success: true,
        playerPoints: player.points,
        malusCount: player.malusCount,
        malusDetail: player.malusDetail
      }));
    });
    return;
  }
  ...
  if (pathname === '/api/admin/reset-game' && req.method === 'POST') {
    parseJsonBody(req, (err, data) => {
      ...
      db.players = allPlayers.map(p => ({
        ...
        sympathyPoints: 0,
        sympathyVotesReceived: [],
        rpgClass: p.rpgClass || "brecher",
        stats: p.stats || { magen: 5, blase: 5, geduld: 5, durst: 5, adrenalin: 5 },
        unlockedTitles: [],
        promilleLogs: [],
        promilleOptIn: p.promilleOptIn !== undefined ? p.promilleOptIn : true
      }));
